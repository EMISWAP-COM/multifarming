...
contract EmiFactory is Ownable {
    ...
    event Deployed(address indexed emiswap, address indexed token1, address indexed token2);
    ...
    function deploy(IERC20 tokenA, IERC20 tokenB) external returns (Emiswap pool) {
        require(address(tokenA) != address(0) && address(tokenB) != address(0), "Factory:no 0x address");
