...
contract EmiRouter {
    ...
    function getPoolDataList(IERC20[] memory tokenAList, IERC20[] memory tokenBList)
        public
        view
        returns (PoolData[] memory dataList)
    {
        if (tokenAList.length > 0 && tokenAList.length == tokenBList.length) {
            ...
            for (uint256 i = 0; i < tokenAList.length; i++) {
                if (
                    address(
                        IEmiswapRegistry(address(factory)).pools(tokenToIERC(tokenAList[i]), tokenToIERC(tokenBList[i]))
                    ) != address(0)
                ) {
                    ...
                    dataList[i].balanceA = IEmiswap(address(dataList[i].pool)).getBalanceForAddition(
                        tokenToIERC(tokenAList[i])
                    );
                    dataList[i].balanceB = IEmiswap(address(dataList[i].pool)).getBalanceForAddition(
                        tokenToIERC(tokenBList[i])
                    );
                }
            }
        ...
        )
    {
        if (address(IEmiswapRegistry(address(factory)).pools(tokenToIERC(token0), tokenToIERC(token1))) != address(0)) {
            ...
            poolAddresss = address(IEmiswapRegistry(address(factory)).pools(tokenToIERC(token0), tokenToIERC(token1)));
        }
    }
    ...
    function getLiquidity(address tokenA, address tokenB) external view returns (uint256) {
        return (IERC20(address(IEmiswapRegistry(factory).pools(IERC20(tokenA), IERC20(tokenB)))).balanceOf(msg.sender));
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = EmiswapLib.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "EmiRouter:INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = EmiswapLib.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "EmiRouter:INSUFFICIENT_A_AMOUNT");
    ...
    function _swap_(
        ...
        (, uint256 amt1) = pairContract.getReturn(IERC20(tokenFrom), IERC20(tokenTo), ammountFrom);
        if (amt1 > 0) {
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, "EmiRouter:INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, "EmiRouter:EXCESSIVE_INPUT_AMOUNT");
    ...
    function swapETHForExactTokens(
        ...
        require(path[0] == WETH, "EmiRouter:INVALID_PATH");
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, "EmiRouter:EXCESSIVE_INPUT_AMOUNT");
